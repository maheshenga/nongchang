...
export function assertRoutingState(current, snapshot, releaseRoot) {
  ...
  if (current.knownStableRelease) {
    const expectedStable = resolve(releaseRoot, 'releases', current.knownStableGitSha);
    if (resolve(current.knownStableRelease) !== expectedStable) {
      throw new Error('known-stable release is outside releases/<40-character-git-sha>');
    }
  }
}
...
export function selectReleasesToPrune(releases, protectedPaths, retainCount = 3) {
  ...
  return safeReleases
    .filter((entry) => !keep.has(resolve(entry.path)))
...
export async function executeReleaseSwitch(candidate, operations) {
  ...
  try {
    ...
    if (candidateStarted) {
      try {
        ...
        // Preserve the primary failure; the inactive candidate is safe to clean up manually.
      }
    }
    throw error;
  }
  ...
  return { status: 'ok', state: next, stateCommitted };
}
...
function parseArgs(argv) {
  ...
  for (const name of ['releaseRoot', 'candidateGitSha', 'candidatePort', 'nodeBin', 'pm2Bin', 'nginxBin', 'nginxConf']) {
