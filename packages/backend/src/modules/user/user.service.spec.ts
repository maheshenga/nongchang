...
beforeEach(() => {
  groups.resolveForCreate.mockReset();
  groups.resolveForCreate.mockResolvedValue('00000000-0000-0000-0000-000000000031');
});
...
describe('UserService.list #27', () => {
  it('agent_admin 有 agentId:where 含 agentId', async () => {
    ...
    await makeService(prisma).list(ctx({ agentId: 'a1' }));
  ...
  it('agent_admin 缺 agentId:抛 Forbidden(不查库)', async () => {
    ...
    await expect(makeService(prisma).list(ctx({ agentId: null }))).rejects.toBeInstanceOf(ForbiddenException);
    expect(prisma.user.findMany).not.toHaveBeenCalled();
  });
  it('system_admin:where 仅 tenantId(允许整租户)', async () => {
    const prisma = { user: { findMany: vi.fn().mockResolvedValue([]) } } as any;
    await makeService(prisma).list(ctx({ role: Role.SYSTEM_ADMIN, agentId: null }));
...
describe('UserService.listPending', () => {
  it('system_admin:查本租户 status=pending', async () => {
    const prisma = { user: { findMany: vi.fn().mockResolvedValue([]) } } as any;
    await makeService(prisma).listPending(ctx({ role: Role.SYSTEM_ADMIN, agentId: null }));
  ...
  it('agent_admin:where 含 agentId + status pending', async () => {
    const prisma = { user: { findMany: vi.fn().mockResolvedValue([]) } } as any;
    await makeService(prisma).listPending(ctx({ agentId: 'a1' }));
    expect(prisma.user.findMany.mock.calls[0][0].where).toMatchObject({ tenantId: 't1', role: Role.MERCHANT, agentId: 'a1', status: 'pending' });
  });
  it('agent_admin 缺 agentId:抛 Forbidden(不查库)', async () => {
    const prisma = { user: { findMany: vi.fn() } } as any;
    await expect(makeService(prisma).listPending(ctx({ agentId: null }))).rejects.toBeInstanceOf(ForbiddenException);
    expect(prisma.user.findMany).not.toHaveBeenCalled();
  });
  it('page/pageSize returns a paginated pending-user envelope', async () => {
    ...
    const result = await makeService(prisma).listPending(
...
describe('UserService.review', () => {
  ...
  it('approve:目标在范围内 → status 置 active', async () => {
    ...
    await makeService(prisma).review(ctx({ role: Role.SYSTEM_ADMIN, agentId: null }), 'p1', { action: 'approve' });
  ...
  it('reject:status 置 rejected', async () => {
    const prisma = { user: { findFirst: vi.fn().mockResolvedValue(pendingUser), update: vi.fn().mockResolvedValue({}) } } as any;
    await makeService(prisma).review(ctx({ role: Role.SYSTEM_ADMIN, agentId: null }), 'p1', { action: 'reject' });
  ...
  it('目标不在范围(跨租户/跨 agent)→ 抛 Forbidden,不更新', async () => {
    ...
    await expect(makeService(prisma).review(ctx({ agentId: 'a1' }), 'pX', { action: 'approve' }))
      .rejects.toBeInstanceOf(ForbiddenException);
    expect(prisma.user.update).not.toHaveBeenCalled();
  });
  it('agent_admin 缺 agentId:抛 Forbidden(不查库)', async () => {
    const prisma = { user: { findFirst: vi.fn(), update: vi.fn() } } as any;
    await expect(makeService(prisma).review(ctx({ agentId: null }), 'p1', { action: 'approve' }))
      .rejects.toBeInstanceOf(ForbiddenException);
