...
function makeService(overrides: any = {}) {
  ...
  const scope = {
    ...
    ownedScopeWhere: vi.fn(async () => ({ tenantId: 't1', ownerId: 'm1' })),
  };
  const prisma = {
  ...
  return {
    ...
    scopeOwnedScopeWhere: scope.ownedScopeWhere,
    calls,
...
describe('FarmRecordService.create 核销', () => {
  ...
  it('completed quota create publishes through the transaction event delegate', async () => {
    ...
    expect(h.scopeOwnedScopeWhere).toHaveBeenCalledWith(h.tx, merchant);
  });
